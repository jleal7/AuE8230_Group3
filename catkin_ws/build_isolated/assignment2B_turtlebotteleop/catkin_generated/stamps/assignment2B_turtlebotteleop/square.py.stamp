...
def square_straight1():
    ...
    while(current_distance <= 1.1):
        ...
        vel.linear.x = rospy.get_param('~v_x')
        vel.linear.y = 0
        vel.linear.z = 0
        vel.angular.x = 0
        vel.angular.y = 0
        ...
        current_distance = rospy.get_param('~v_x')*(t1-t0)
    ...
    while(current_angle<=1.5707963267948): # angle is in radians
        ...
        vel.angular.x = 0
        vel.angular.y = 0
        vel.angular.z = rospy.get_param('~w_z')
            
        pub.publish(vel)
        t3 = rospy.Time.now().to_sec() # current time in the loop
        current_angle = rospy.get_param('~w_z')*(t3-t2) # current angle
    ...
    while(current_angle<=1.5707963267948):
        ...
        current_angle = rospy.get_param('~w_z')*(t3-t2)
